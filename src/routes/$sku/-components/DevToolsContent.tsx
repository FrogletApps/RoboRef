...
const EventDevToolsContent: React.FC = () => {
        ...
        for (let i = 0; i < generateIssueNumber; i++) {
          const team =
            teamsInDivision!.teams[
              getSecureRandomInt(teamsInDivision!.teams.length)
            ];

          const match =
            matchesInDivision?.[
              getSecureRandomInt(matchesInDivision.length)
            ];

          const rule = rules?.[getSecureRandomInt(rules.length)];
          ...
          const outcome = OUTCOMES[getSecureRandomInt(OUTCOMES.length)];
  ...
  const { mutate: deleteIncidents, isPending: isDeletePending } = useMutation({
    mutationFn: async () => {
      ...
      const toDelete = shuffled.slice(0, incidentsToDelete);
...
export default EventDevToolsContent;
