        ...
        for (let i = 0; i < generateIssueNumber; i++) {
          const team =
            teamsInDivision!.teams[
              Math.floor(getRandom() * teamsInDivision!.teams.length)
            ];

          const match =
            matchesInDivision?.[
              Math.floor(getRandom() * matchesInDivision.length)
            ];

          const rule = rules?.[Math.floor(getRandom() * rules.length)];
          ...
          const outcome = OUTCOMES[Math.floor(getRandom() * OUTCOMES.length)];
