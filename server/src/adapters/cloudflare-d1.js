export class CloudflareD1Adapter {
    ...
    async getEvent(sku) {
        const result = await this.db.prepare("SELECT * FROM events WHERE sku = ?").bind(sku).first();
        return result || null;
    }
    ...
    `).bind(event.sku, event.name, event.program, event.season, event.startDate, event.endDate, event.updatedAt).run();
    }
    async getNotesSince(sku, sinceVersion) {
        const { results } = await this.db.prepare("SELECT * FROM notes WHERE sku = ? AND version > ? ORDER BY version ASC").bind(sku, sinceVersion).all();
    ...
    async applyNoteChanges(sku, changes) {
        const maxVerRow = await this.db.prepare("SELECT MAX(version) as maxVer FROM notes WHERE sku = ?").bind(sku).first();
    ...
    `;
        for (let i = 0; i < changes.length; i += D1_BATCH_LIMIT) {
            ...
            for (const record of chunk) {
                currentMax += 1;
                statements.push(this.db.prepare(insertSql).bind(record.id, record.sku, record.teamNumber, record.matchId ?? null, JSON.stringify(record.ruleCodes || []), record.severity, record.notes, record.refereeName, record.deviceId, record.createdAt, record.updatedAt, record.isDeleted ? 1 : 0, currentMax));
            }
    ...
    `).bind(session.id, session.sku, session.adminDeviceId, session.adminRefereeName, session.createdAt, session.updatedAt, JSON.stringify(session.participants)).run();
        return session;
    }
    async getShareSession(id) {
        const row = await this.db.prepare("SELECT * FROM share_sessions WHERE id = ?").bind(id).first();
        if (!row)
            return null;
        return this.mapShareRow(row);
    }
    async getActiveSharesForSku(sku) {
        const { results } = await this.db.prepare("SELECT * FROM share_sessions WHERE sku = ? ORDER BY createdAt DESC").bind(sku).all();
    ...
    async addParticipant(shareId, participant) {
        ...
        await this.db.prepare("UPDATE share_sessions SET participantsJson = ?, updatedAt = ? WHERE id = ?").bind(JSON.stringify(session.participants), session.updatedAt, shareId).run();
        return session;
    }
