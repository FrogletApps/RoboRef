...
import { VexEventsClient, Match } from "@roboref/vexevents";
...
export function makeMatchComparison(matches: Match[]) {
  return function matchComparison(
    ...
    if (a.type === "match" && b.type === "match") {
      const realA = matches.find((m) => m.id === a.id);
      const realB = matches.find((m) => m.id === b.id);

      if (realA && realB) {
        if (realA.division.id !== realB.division.id) {
          return realA.division.id - realB.division.id;
        }
        if (realA.round !== realB.round) {
          return realA.round - realB.round;
        }
        if (realA.instance !== realB.instance) {
          return realA.instance - realB.instance;
        }
        if (realA.matchnum !== realB.matchnum) {
          return realA.matchnum - realB.matchnum;
        }
        return a.name.localeCompare(b.name);
      }

      // Fallback if real matches are not found
...
export function makeIncidentComparison(matches: Match[]) {
  const matchComparison = makeMatchComparison(matches);

  return function incidentComparison(a: Incident, b: Incident): number {
...
export async function generateIncidentReportPDF({
  ...
  let matches: Match[] = [];
  if (event.data.divisions) {
    const divisionMatches = await Promise.all(
      event.data.divisions.map(async (division) => {
        if (division.id) {
          const res = await event.data!.matches(division.id);
          if (res.data) return res.data;
        }
        return [];
      }),
    );
    matches = divisionMatches.flat();
  }

  const data: IncidentRow[] = [];

  const incidentComparison = makeIncidentComparison(matches);
