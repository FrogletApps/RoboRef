...
- **Changelog Requirement**: Whenever making a change that affects the end user (UI modifications, new features, bug fixes impacting user interaction, UX improvements), you **MUST** add an entry to `app/assets/changeLog.md` under a heading with the current version or date and build number (e.g. `## 2026.8.27+1`). This should be as brief as possible.
